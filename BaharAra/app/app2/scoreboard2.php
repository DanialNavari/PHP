...
function visitor_earn()
{
    ...
    for ($i = 0; $i < $n; $i++) {
        ...
        $start = $GLOBALS['start'];
        $end = $GLOBALS['end'];
        $s = "SELECT SUM(fee) AS total FROM `allfactors` WHERE visitor = '$visitor' AND mali = 'تسويه نشده' AND tarikh>= $start AND tarikh<=$end";
...
<table style="margin: 0.5rem auto 0;">
    <tr>
        <td colspan="3" style="border: none;">
            <button class="btn btn-info" id="return" onclick="open_page('amar_notpay')">بازگشت</button>
    ...
    <tr>
        <td>رتبه</td>
        <td>ویزیتور</td>
        <td>تسویه نشده <?php echo $when; ?>(ریال)</td>
    </tr>
    <?php
    visitor_earn();
