...
function order_info_($operator, $order_type, $zaman)
{
    ...
    if ($operator == 'LIKE') {
        ...
        $st = '`accept`="" OR `accept` IS ' . $order_type . ' AND `buy_pos` = "+" AND `logout` IS NOT NULL OR `logout` !=""';
    }

    $sql = "SELECT * FROM `cbd` WHERE `login` LIKE '%" . $zaman . "%' AND $st ORDER BY `id` desc";
    ...
    for ($i = 0; $i < $num; $i++) {
        ...
        //if ($lines == '1' || isset($GLOBALS['admin'])) {
        get_factor_($row['shop_id']);
...
order_info($zaman);
//order_info_('empty', 'NULL', $_GET['date']);
                                        ...
                                        <tr>
                                            ...
                                            for ($i = 0; $i < $count; $i++) {
                                                ...
                                                if ($check_is_factor > 0) {
                                                    $factor_link = "<a target='_blank' href='factor.php?f=" . $f_id . "&d=" . $_login[0] . "'>" . $factor_id . "</a>";
                                                } else {
                                                    $factor_link = "-";
                                                }
