...
function visitor_earn()
{
    ...
    for ($i = 0; $i < $n; $i++) {
        ...
        $start = $GLOBALS['start'];
        $end = $GLOBALS['end'];
        $s = "SELECT SUM(fee) AS total FROM `allfactors` WHERE visitor = '$visitor' AND tarikh>= $start AND tarikh<=$end";
...
function visitor_marju($visitor_name, $start, $end)
{
    db();
    $sql = "SELECT SUM(fee) AS total FROM `marju` WHERE visitor = '$visitor_name' AND tarikh>= $start AND tarikh<=$end";
    $w = mysqli_query($GLOBALS['conn'], $sql);
    $n = mysqli_num_rows($w);
    for ($i = 0; $i < $n; $i++) {
        $r = mysqli_fetch_assoc($w);
...
<table style="margin: 0.5rem auto 0;">
    <tr>
        <td colspan="3" style="border: none;">
            <button class="btn btn-info" id="return" onclick="open_page('amar_sell')">بازگشت</button>
