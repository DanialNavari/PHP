<div class="row empty">دوره ها > دوره های خاتمه یافته > مسافرت جنوب > گزارش</div>
<div class="cat">
    <div class="card my_card">
        <table class="table">
            <tr class="bg_dark_blue">
                <td class="td_title font-weight-bold text-white" colspan="1">نام دوره</td>
                <td class="td_title_ text-white text-" colspan="3"><span>مسافرت جنوب</span><span>(1403/03/01)</span></td>
            </tr>
            <tr class="bg_dark_blue">
                <td class="td_title font-weight-bold text-white" colspan="1">خرج کرد</td>
                <td class="td_title_ text-white d-rtl" colspan="3"><span>15,000,000 </span><span class="unit">ريال</span></td>
            </tr>
            <tr class="bg_blue">
                <td class="td_title_ font-weight-bold  text-white va_middle text-center">نام</td>
                <td class="td_title_ font-weight-bold text-center text-white d-rtl va_middle text-center">خرج کرد (ريال)</td>
                <td class="td_title_ font-weight-bold text-center text-white d-rtl va_middle text-center">سهم (ريال)</td>
                <td class="td_title_ font-weight-bold text-center text-white d-rtl va_middle text-center">وضعیت</td>
            </tr>
            <tr class="bg_grey">
                <td class="td_title text-primary">اشکان توکلی</td>
                <td class="td_title_ text-primary text-center">3,000,000</td>
                <td class="td_title_ text-primary text-center">5,000,000</td>
                <td class="td_title_ text-primary text-center bedehkar">بدهکار</td>
            </tr>
            <tr class="bg_blue_nice">
                <td class="td_title_ text-primary text-left" colspan="4">3,000,000</td>
            </tr>
            <tr class="bg_grey">
                <td class="td_title text-primary">دانیال نواری</td>
                <td class="td_title_ text-primary text-center">4,000,000</td>
                <td class="td_title_ text-primary text-center">5,000,000</td>
                <td class="td_title_ text-primary text-center bedehkar">بدهکار</td>
            </tr>
            <tr class="bg_blue_nice">
                <td class="td_title_ text-primary text-left" colspan="4">1,000,000</td>
            </tr>
            <tr class="bg_grey">
                <td class="td_title text-primary">علیرضا صالحی</td>
                <td class="td_title_ text-primary text-center">9,000,000</td>
                <td class="td_title_ text-primary text-center">5,000,000</td>
                <td class="td_title_ text-primary text-center talabkar">طلبکار</td>
            </tr>
            <tr class="bg_blue_nice">
                <td class="td_title_ text-primary text-left" colspan="4">4,000,000</td>
            </tr>
        </table>
        <div class="pay_btn pay_btn2" onclick="window.print()">
            <div class="pay_btn_icon">
                <?php echo $symbol; ?>
            </div>
        </div>
    </div>
</div>